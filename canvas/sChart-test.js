...
sBarChart.prototype = {
    ...
    looping2: function() {
        this.looped2 = requestAnimationFrame(this.looping2.bind(this));
        if(this.current2 < 100){
            this.current2 = (this.current2 + 2) >= 100 ? 0 : (this.current2 + 2);
        }else{
            //this.current2=0;
        }
        this.drawAnimation2();
    },
    drawAnimation2: function() {
        for(var i = 0; i < this.dataLength; i++) {
            var x = Math.ceil(this.data[i].value * this.current2 / 100 * this.yRatio);
            var y = this.height - this.padding - x;
            //console.log(y);
